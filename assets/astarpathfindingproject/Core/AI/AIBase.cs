	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		protected virtual void FixedUpdate () {
            if (!(rigid == null && rigid2D == null) && canMove) {
                ...
                //Debug.Log("positionX dif: " + (nextPosition.x - tr.position.x) + ", positionY dif:" + (nextPosition.y - tr.position.y));

                // code for the animation of companion
                Animator animator = gameObject.GetComponent<Animator>();
                if (nextPosition.x - tr.position.x != 0)
                {
                    if (nextPosition.x - tr.position.x < 0)
                        animator.SetTrigger("walkingLeft");
                    else if (nextPosition.x - tr.position.x > 0)
                        animator.SetTrigger("walkingRight");
                }
                else if (nextPosition.y - tr.position.y != 0)
                {
                    if (nextPosition.y - tr.position.y < 0)
